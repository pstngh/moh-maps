...
The negative-determinant transform required format-specific handling. For brushes, every face point's X coordinate is negated and points zero and two are exchanged so the plane/inside direction remains valid. For legacy MOH `patchDef2`, the serialized first dimension is row count and the second is controls per row; every control X is reflected and the row order is reversed, matching NetRadiant's left-handed patch inversion. For `terrainDef`, reflecting the grid changes the origin to `-(x0 + (width - 1) * 64)`, reverses each material-control and sample row, and exchanges the two triangle-flag groups attached to each sample. Simply negating the terrain origin would move the grid instead of mirroring it in place.
