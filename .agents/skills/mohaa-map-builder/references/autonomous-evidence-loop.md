...
substituted under the wrong role; reject such internally mixed evidence. Bundle
verification must also reopen the content-addressed candidate PK3, require its
one exact case-sensitive expected BSP member, rehash the complete member
inventory and inner BSP, and compare those observations with the manifest and
audit. Do not trust package structure merely because the audit and manifest were
rehashed together. Repeating unchanged inputs must produce the same manifest hash
...
Require the bundled evidence plan's map name and expected BSP member to match
