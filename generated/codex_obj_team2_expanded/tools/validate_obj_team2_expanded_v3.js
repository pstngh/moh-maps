...
assert(report.schemaVersion === 3 && report.revision === 3, "Expected revision 3 generation report");
...
const expectedFoliage = new Set([453, 687, 688, 689, 690, 692, 693, 699, 702, 704]);
const expectedEastFenceEntities = new Set(Array.from({ length: 19 }, (_value, index) => 666 + index));
const expectedAlliedGateEntities = new Set([442, 444]);
const expectedRemovedEntities = new Set([...expectedFoliage, ...expectedEastFenceEntities, ...expectedAlliedGateEntities]);
const expectedEastFenceWorldBrushes = new Set([1109, 1110, 1111, 1112, 1113, 1114, 1115, 1124]);
const expectedAlliedGateWorldBrushes = new Set([819, 3224, 3228, 3229]);
const expectedRemovedWorldBrushes = new Set([...expectedEastFenceWorldBrushes, ...expectedAlliedGateWorldBrushes]);
assert(JSON.stringify([...report.preservation.removedEntityNumbers].sort((a, b) => a - b)) === JSON.stringify([...expectedRemovedEntities].sort((a, b) => a - b)), "Removed entity inventory differs from revision-3 policy");
assert(JSON.stringify([...report.preservation.removedWorldBrushNumbers].sort((a, b) => a - b)) === JSON.stringify([...expectedRemovedWorldBrushes].sort((a, b) => a - b)), "Removed world-brush inventory differs from revision-3 policy");
...
for (const entity of sourceEntities.filter(({ number }) => expectedRemovedEntities.has(number))) {
  ...
  const boundary = (expectedEastFenceEntities.has(entity.number) || expectedAlliedGateEntities.has(entity.number)) && ["detail", "func_group"].includes(entity.keys.classname);
  assert((foliage || boundary) && !entity.keys.targetname, `Entity ${entity.number} violates the documented removal policy`);
  for (let index = entity.markerIndex; index <= entity.closeIndex; index += 1) removedIndexes.add(index);
}
for (const brush of sourceWorldBrushes.filter(({ number }) => expectedRemovedWorldBrushes.has(number))) {
  for (let index = brush.markerIndex; index <= brush.closeIndex; index += 1) removedIndexes.add(index);
}
assert([...expectedRemovedWorldBrushes].every((number) => sourceWorldBrushes.some((brush) => brush.number === number)), "A removed stock world brush is absent");
...
assert(customEntities.filter((entity) => entity.keys.classname === "info_player_deathmatch").length === 12, "Expected twelve revision-3 DM spawns");
...
const forbiddenMaterials = ["common/caulk", "common/nodraw", "general_industrial/deckgrate_set1a", "general_industrial/deckgrate_set1b"];
assert(!customWorldLines.some((line) => forbiddenMaterials.some((texture) => line.includes(` ${texture} `))), "Revision 3 contains an invisible or alpha construction face");
assert(report.expansion.fullySkinnedBrushes === report.expansion.brushes.length, "Not every expansion brush is fully skinned");
assert(report.expansion.transparentConstructionMaterials.length === 0, "Transparent construction material remains in revision 3");
for (const brush of report.expansion.brushes) {
  ...
  assert(brush.min[0] >= -2432 && brush.max[0] <= 5152 && brush.min[1] >= 0 && brush.max[1] <= 2496 && brush.min[2] >= -560 && brush.max[2] <= 336, `${brush.role} escaped the surveyed revision-3 bounds`);
}

const roles = report.expansion.brushRoleCounts;
assert(roles.service_deck === 2 && roles.grand_apron_steps === 6, "Leak-safe L deck or continuous frontage inventory changed");
assert(roles.south_workshop_roof === 1 && roles.north_workshop_roof === 1, "Both three-bay workshops are required");
assert(roles.hall_bay_awning === 4 && roles.hall_ceiling_beam === 8, "Four-bay hall completion inventory changed");
assert(roles.allied_fence_base_south === 1 && roles.allied_fence_base_north === 1, "Allied fence curb replacement is incomplete");
assert(roles.allied_gate_apron === 1 && roles.allied_outer_lane === 1 && roles.allied_rear_link === 1, "Allied exterior route is incomplete");
assert(roles.allied_route_light_pole === 4, "Allied exterior route lighting supports changed");

const apron = report.expansion.brushes.filter((brush) => brush.role === "grand_apron_steps").sort((a, b) => a.min[0] - b.min[0]);
assert(apron.length === 6 && apron[0].min[0] === 3040 && apron[5].max[0] === 3232, "Grand apron no longer bridges stock ground to the deck");
assert(apron.every((brush) => brush.min[1] === 384 && brush.max[1] === 2208), "Grand apron lost its 1,824-unit continuous width");
assert(apron.every((brush, index) => index === 0 || brush.max[2] - apron[index - 1].max[2] === 16), "Grand apron risers changed");
assert(report.expansion.continuousFrontageCount === 1 && report.expansion.continuousFrontageWidth === 1824, "Continuous frontage metadata changed");
assert(report.expansion.openHallBays === 4 && report.expansion.alliedGateWidth === 326 && report.expansion.alliedExteriorRouteConnected, "Route/access metadata changed");
...
process.stdout.write(`${JSON.stringify({
  ...
  removedEastFenceEntities: expectedEastFenceEntities.size,
  removedAlliedGateEntities: expectedAlliedGateEntities.size,
  removedWorldBrushes: expectedRemovedWorldBrushes.size,
  addedBrushes: brushMarkers.length,
  addedEntities: customEntities.length,
  dmSpawns: dmSpawns.length,
  continuousFrontageWidth: report.expansion.continuousFrontageWidth,
  alliedGateWidth: report.expansion.alliedGateWidth,
  openHallBays: report.expansion.openHallBays,
  customAssetBytes: 0,
}, null, 2)}\n`);
