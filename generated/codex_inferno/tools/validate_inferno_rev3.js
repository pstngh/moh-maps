...
expect(report.revision === 3, "Expected Inferno revision 3");
expect(
  report.construction === "callout-zoned-complete-village-massing-over-measured-routes",
...
expect(
  report.privateReferenceEvidence.officialOverview.posX === -2087 &&
    report.privateReferenceEvidence.officialOverview.posY === 3870 &&
    report.privateReferenceEvidence.officialOverview.scale === 4.9,
  "Official overview transform evidence changed"
);
expect(
  report.privateReferenceEvidence.verifiedLandmarkDimensions.bFountain
    .basinRadius === 156 &&
    report.privateReferenceEvidence.verifiedLandmarkDimensions.bFountain
      .centerRadius === 43 &&
    report.privateReferenceEvidence.verifiedLandmarkDimensions.ctWell
      .baseRadius === 59,
  "Verified hero-landmark dimensions changed"
);
expect(
  report.privateReferenceEvidence.verifiedDynamicDoors === 1,
  "Verified Source dynamic-door count changed"
);
expect(
...
expect(
  report.geometry.massColumns >= 9000 &&
    report.geometry.buildingMassRectangles >= 300 &&
    report.geometry.buildingMassRectangles < 700,
  "Complete village massing left its measured range"
);
expect(
  report.geometry.renderedOutdoorWallRuns === 0,
  "Rejected outdoor boundary strips were rendered again"
);
expect(
  report.geometry.renderedIndoorWallRuns >= 100 &&
    report.geometry.renderedIndoorWallRuns < 400,
  "Indoor separation-wall count left its measured range"
);
expect(
  report.geometry.gableRoofs >= 16 && report.geometry.gableRoofs <= 72,
  "Semantic roof count left its authored range"
);
expect(
  report.geometry.totalWorldspawnBrushes >= 1500 &&
