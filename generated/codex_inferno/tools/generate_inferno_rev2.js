...
for (let index = 0; index < 12; index++) {
  ...
  fountainOuter.push([
    fountainX + Math.cos(angle) * 104,
    fountainY + Math.sin(angle) * 104,
  ]);
  fountainInner.push([
    fountainX + Math.cos(angle) * 38,
    fountainY + Math.sin(angle) * 38,
  ]);
}
...
const worldspawn = [
  ...
  `"message" "Codex Inferno - measured clone revision 2"`,
  `"ambientlight" "8 9 12"`,
...
setcvar "g_obj_alliedtext2" "Measured reconstruction - revision 2"
...
const report = {
  map: mapName,
  revision: 2,
  construction: "manually-reauthored-clone-from-measured-topology",
  referencePolicy:
...
const svg = [
  ...
  `<circle cx="${fountainX}" cy="${-fountainY}" r="104" fill="#6e9aa3" stroke="#e5d8c3" stroke-width="18"/>`,
  ...calloutAnchors.map(
    (anchor) =>
  ...
  `<text x="${coordinateBounds.minX + 80}" y="${-coordinateBounds.maxY + 110}" font-family="sans-serif" font-size="64" font-weight="700" fill="#ffffff">Codex Inferno revision 2 — measured clone footprint</text>`,
  "</svg>",
];
