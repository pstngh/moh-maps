...
const modelBoundsManifest = JSON.parse(
  fs.readFileSync(path.join(__dirname, "..", "inferno-model-bounds.json"), "utf8")
);
const modelBoundsByPath = new Map(
  modelBoundsManifest.models.map((record) => [record.model.toLowerCase(), record])
);
...
function transformLocalPoint(origin, yaw, point) {
...
function transformLocalPoint3D(origin, angles, point) {
  const pitch = ((angles[0] || 0) * Math.PI) / 180;
  const yaw = ((angles[1] || 0) * Math.PI) / 180;
  const roll = ((angles[2] || 0) * Math.PI) / 180;
  ...
  const rolledY = point[1] * cosR - point[2] * sinR;
  const rolledZ = point[1] * sinR + point[2] * cosR;
  const pitchedX = point[0] * cosP + rolledZ * sinP;
  const pitchedZ = -point[0] * sinP + rolledZ * cosP;
  return [
    origin[0] + pitchedX * cosY - rolledY * sinY,
    origin[1] + pitchedX * sinY + rolledY * cosY,
    origin[2] + pitchedZ,
  ];
}

function expandedLocalBounds(localMin, localMax, minimumExtent = 2, scale = 1) {
  const min = localMin.map((coordinate) => coordinate * scale);
  const max = localMax.map((coordinate) => coordinate * scale);
  for (let axis = 0; axis < 3; axis++) {
    const extent = max[axis] - min[axis];
    if (extent >= minimumExtent) continue;
    const center = (min[axis] + max[axis]) / 2;
    min[axis] = center - minimumExtent / 2;
    max[axis] = center + minimumExtent / 2;
  }
  return { min, max };
}

function orientedBoxBrush(
  origin,
  yaw,
  localMin,
  localMax,
  ...
  const bounds = expandedLocalBounds(localMin, localMax, minimumExtent, scale);
  ...
  const points = Object.fromEntries(
    Object.entries(local).map(([key, point]) => [
      key,
      transformLocalPoint(origin, yaw, point),
    ])
  );
  return [
    "{",
    face([points.p000, points.p010, points.p011], material, isDetail, surfaceParms),
    face([points.p100, points.p101, points.p111], material, isDetail, surfaceParms),
    face([points.p000, points.p001, points.p101], material, isDetail, surfaceParms),
    face([points.p010, points.p110, points.p111], material, isDetail, surfaceParms),
    face([points.p000, points.p100, points.p110], material, isDetail, surfaceParms),
    face([points.p001, points.p011, points.p111], material, isDetail, surfaceParms),
    "}",
  ].join("\n");
}

function orientedBoxBrush3D(
  origin,
  angles,
  localMin,
  localMax,
  ...
  const [maxX, maxY, maxZ] = bounds.max;
  const local = {
    p000: [minX, minY, minZ], p001: [minX, minY, maxZ],
    p010: [minX, maxY, minZ], p011: [minX, maxY, maxZ],
    p100: [maxX, minY, minZ], p101: [maxX, minY, maxZ],
    p110: [maxX, maxY, minZ], p111: [maxX, maxY, maxZ],
  };
  const points = Object.fromEntries(
    Object.entries(local).map(([key, point]) => [
      key,
      transformLocalPoint3D(origin, angles, point),
    ])
  );
  return [
    "{",
...
function convexBrush(facePolygons, center, material, surfaceParms = "", isDetail = true) {
  const lines = ["{"];
  for (const polygon of facePolygons) {
    ...
    if (dot(normal, subtract(center, a)) < 0) [b, c] = [c, b];
    lines.push(face([a, b, c], material, isDetail, surfaceParms));
  }
  lines.push("}");
  return lines.join("\n");
}

function polygonPrismBrush(points, minZ, maxZ, material, surfaceParms = "") {
  const center = [
  ...
  return convexBrush(polygons, center, material, surfaceParms);
}

function annularRingBrushes(origin, minZ, maxZ, outerRadius, innerRadius, material, sides = 12) {
  const brushes = [];
  for (let index = 0; index < sides; index++) {
    const a0 = (index / sides) * Math.PI * 2;
    const a1 = ((index + 1) / sides) * Math.PI * 2;
    const points = [
      [origin[0] + Math.cos(a0) * outerRadius, origin[1] + Math.sin(a0) * outerRadius],
      [origin[0] + Math.cos(a1) * outerRadius, origin[1] + Math.sin(a1) * outerRadius],
      [origin[0] + Math.cos(a1) * innerRadius, origin[1] + Math.sin(a1) * innerRadius],
      [origin[0] + Math.cos(a0) * innerRadius, origin[1] + Math.sin(a0) * innerRadius],
    ];
    brushes.push(polygonPrismBrush(points, minZ, maxZ, material));
  }
  return brushes;
}

function sourceModelScale(entity) {
  for (const key of ["uniformscale", "modelscale", "scale"]) {
    const parsed = Number(value(entity.children, key));
...
function groundedModelOrigin(origin, record, scale, surfaces, stats) {
  const probe = [origin[0], origin[1], origin[2] + record.hullMin[2] * scale];
  const grounding = snapOriginToPlanarSupport(probe, surfaces, stats);
  return [
    origin[0],
    origin[1],
    grounding.origin[2] - record.hullMin[2] * scale,
  ];
}

function modelFrameBrushes(origin, yaw, record, material, scale = 1) {
  const bounds = expandedLocalBounds(record.hullMin, record.hullMax, 4, scale);
  const extentX = bounds.max[0] - bounds.min[0];
  const extentY = bounds.max[1] - bounds.min[1];
  const widthAxis = extentX >= extentY ? 0 : 1;
  const frameWidth = Math.max(10, Math.min(24, Math.max(extentX, extentY) * 0.1));
  const leftMin = [...bounds.min];
  const leftMax = [...bounds.max];
  leftMax[widthAxis] = leftMin[widthAxis] + frameWidth;
  const rightMin = [...bounds.min];
  const rightMax = [...bounds.max];
  rightMin[widthAxis] = rightMax[widthAxis] - frameWidth;
  const topMin = [...bounds.min];
  const topMax = [...bounds.max];
  topMin[2] = topMax[2] - frameWidth;
  return [
    orientedBoxBrush(origin, yaw, leftMin, leftMax, material, "-surfaceparm solid"),
    orientedBoxBrush(origin, yaw, rightMin, rightMax, material, "-surfaceparm solid"),
    orientedBoxBrush(origin, yaw, topMin, topMax, material, "-surfaceparm solid"),
  ];
}
...
const stats = {
  ...
  sourcePropCandidates: 0,
  sourcePropsWithBounds: 0,
  sourcePropsSubstituted: 0,
  propSubstituteBrushes: 0,
  facadeWindows: 0,
  facadeShutters: 0,
  facadeDoors: 0,
  facadeFrames: 0,
  measuredArchFrames: 0,
  roofSurfaceProps: 0,
  measuredPillars: 0,
  measuredChimneys: 0,
  measuredWoodSupports: 0,
  measuredBalconySupports: 0,
  measuredCoverProps: 0,
  landmarkAssemblies: 0,
  collapsedLandmarkParts: 0,
  tiltedPropsOmitted: 0,
  sourceSkySkipped: 0,
...
// Reintroduce the high-impact prop layer using only verified IDST hull bounds.
// Facade, roof, trim, and skyline substitutes are non-solid so approximate art
// cannot close routes. Only measured cover and landmark bodies receive solid
// collision. No Source model bytes are emitted or packaged.
const generatedLandmarkKeys = new Set();
const propSubstitutionRecords = [];
function landmarkKey(kind, origin) {
  return `${kind}:${Math.round(origin[0])}:${Math.round(origin[1])}:${Math.round(origin[2])}`;
}
function appendPropBrushes(brushes) {
  const list = Array.isArray(brushes) ? brushes : [brushes];
  worldBrushes.push(...list);
  stats.propSubstituteBrushes += list.length;
}
...
for (const entity of sourceEntities) {
  ...
  const model = value(entity.children, "model").toLowerCase().replace(/\\/g, "/");
  const origin = parseVector(value(entity.children, "origin", "0 0 -9999"));
  if (!inPlayableArea(origin)) continue;
  stats.sourcePropCandidates++;
  const record = modelBoundsByPath.get(model);
  if (!record) {
    stats.unsupportedPropsSkipped++;
    stats.unverifiedPropsOmitted++;
    continue;
  }
  stats.sourcePropsWithBounds++;
  const angles = parseVector(value(entity.children, "angles", "0 0 0"));
  const pitch = Number.isFinite(angles[0]) ? normalizedAngle(angles[0]) : 0;
  const yaw = Number.isFinite(angles[1]) ? angles[1] : 0;
  const roll = Number.isFinite(angles[2]) ? normalizedAngle(angles[2]) : 0;
  const scale = sourceModelScale(entity);
  const propBrushStart = stats.propSubstituteBrushes;
  let substituted = false;

  const addMeasuredBox = (material, surfaceParms = "-surfaceparm solid", grounded = false) => {
    if (Math.abs(pitch) > 5 || Math.abs(roll) > 5) {
      stats.tiltedPropsOmitted++;
      return false;
    }
    const placedOrigin = grounded
      ? groundedModelOrigin(origin, record, scale, planarDisplacementSupports, stats)
      : origin;
    appendPropBrushes(
      orientedBoxBrush(
        placedOrigin,
  ...
  if (
    /\/window_b\/window_b\.mdl$|\/window_collection\/window_c\.mdl$|\/window_d\/window_d\.mdl$|\/window_frame_a\/window_frame_a\.mdl$|\/church_window\/|\/inferno_circular_window\//.test(model)
  ) {
    substituted = addMeasuredBox(targetMaterials.windowBacking);
    if (substituted) stats.facadeWindows++;
  } else if (/\/inferno_folding_shutters\//.test(model)) {
    substituted = addMeasuredBox(targetMaterials.wood);
    if (substituted) stats.facadeShutters++;
  } else if (
    /\/door_a\/door_a\.mdl$|\/inferno_door_single\/|\/inverno_balcony_door\/|\/door_collection\/door_collection_[bd]\.mdl$|\/store_front\/store_front_door\.mdl$|\/roll_up_door\/roll_up_door_b_(full|02)\.mdl$/.test(model)
  ) {
    substituted = addMeasuredBox(
      model.includes("roll_up_door") ? targetMaterials.paintedMetal : targetMaterials.wood
    );
    if (substituted) stats.facadeDoors++;
  } else if (/\/door_frame\/|\/door_collection\/door_collection_trim_|\/ornate_door_frame\//.test(model)) {
    if (Math.abs(pitch) <= 5 && Math.abs(roll) <= 5) {
      appendPropBrushes(modelFrameBrushes(origin, yaw, record, targetMaterials.concreteDark, scale));
      stats.facadeFrames++;
      substituted = true;
    }
  } else if (/\/arch_[abcde]\//.test(model)) {
    if (Math.abs(pitch) <= 5 && Math.abs(roll) <= 5) {
      appendPropBrushes(modelFrameBrushes(origin, yaw, record, targetMaterials.concreteDark, scale));
      stats.measuredArchFrames++;
      substituted = true;
    }
  } else if (/\/tile_set_a\/tile_set_a(?:3)?\.mdl$/.test(model)) {
    appendPropBrushes(
      orientedBoxBrush3D(
        origin, angles, record.hullMin, record.hullMax,
        targetMaterials.corrugatedGray, "-surfaceparm solid", true, scale, 2
      )
    );
    stats.roofSurfaceProps++;
    substituted = true;
  } else if (/\/inferno_window_overhang\//.test(model)) {
    appendPropBrushes(
      orientedBoxBrush3D(
        origin, angles, record.hullMin, record.hullMax,
        targetMaterials.corrugatedGray, "-surfaceparm solid", true, scale, 2
    ...
    stats.roofSurfaceProps++;
    substituted = true;
  } else if (/\/pillar_[ab]\/|\/church_pillar\//.test(model)) {
    substituted = addMeasuredBox(targetMaterials.concreteDark);
    if (substituted) stats.measuredPillars++;
  } else if (/\/chimney\/|\/inferno_chimney\/|models\/props_exteriors\/chimney4\.mdl$/.test(model)) {
    substituted = addMeasuredBox(targetMaterials.brick);
    if (substituted) stats.measuredChimneys++;
  } else if (/\/wood_supports_a\//.test(model)) {
    appendPropBrushes(
      orientedBoxBrush3D(
        origin, angles, record.hullMin, record.hullMax,
        targetMaterials.wood, "-surfaceparm solid", true, scale, 3
      )
    );
    stats.measuredWoodSupports++;
    substituted = true;
  } else if (/\/inferno_balcony\/inferno_balcony_support/.test(model)) {
    substituted = addMeasuredBox(targetMaterials.wood);
    if (substituted) stats.measuredBalconySupports++;
  } else if (/\/barrel_a\/barrel_a_full\.mdl$|\/barrel_b\/barrel_b\.mdl$/.test(model)) {
    if (Math.abs(pitch) <= 5 && Math.abs(roll) <= 5) {
      const placedOrigin = groundedModelOrigin(origin, record, scale, planarDisplacementSupports, stats);
      const radius = Math.max(
        record.hullMax[0] - record.hullMin[0],
        record.hullMax[1] - record.hullMin[1]
      ) * scale / 2;
      appendPropBrushes(
        cylinderBrush(
          placedOrigin,
          placedOrigin[2] + record.hullMin[2] * scale,
          placedOrigin[2] + record.hullMax[2] * scale,
          radius,
          targetMaterials.paintedMetal
        )
      );
      stats.barrels++;
      stats.measuredCoverProps++;
      substituted = true;
    }
  } else if (
    /\/wine_crate_a\/|\/weapon_crate_a\/|\/inferno_wine_crate\//.test(model)
  ) {
    substituted = addMeasuredBox(
      model.includes("weapon_crate") ? targetMaterials.paintedMetal : targetMaterials.wood,
      "",
      true
    );
    if (substituted) stats.measuredCoverProps++;
  } else if (/\/hay_bale\/hay_bale_a\.mdl$/.test(model)) {
    substituted = addMeasuredBox(targetMaterials.grass, "", true);
    if (substituted) {
      stats.hayBales++;
      stats.measuredCoverProps++;
    }
  } else if (/\/coffin\/inferno_coffin\.mdl$/.test(model)) {
    substituted = addMeasuredBox(targetMaterials.wood, "", true);
    if (substituted) stats.measuredCoverProps++;
  } else if (/\/fountain_a_basin\//.test(model)) {
    const key = landmarkKey("fountain-basin", origin);
    if (!generatedLandmarkKeys.has(key)) {
      generatedLandmarkKeys.add(key);
      appendPropBrushes(
        annularRingBrushes(
          origin,
          origin[2] - 15,
      ...
      stats.landmarkAssemblies++;
    } else {
      stats.collapsedLandmarkParts++;
    }
    substituted = true;
  } else if (/\/fountain_a\/fountain_a(?:_water)?\.mdl$/.test(model)) {
    const key = landmarkKey("fountain-center", origin);
    if (!generatedLandmarkKeys.has(key)) {
      generatedLandmarkKeys.add(key);
      appendPropBrushes(
        cylinderBrush(origin, origin[2], origin[2] + 133, 43, targetMaterials.concreteDark, 12)
      );
      stats.landmarkAssemblies++;
    } else {
      stats.collapsedLandmarkParts++;
    ...
    substituted = true;
  } else if (/\/well\//.test(model)) {
    const key = landmarkKey("well", origin);
    if (!generatedLandmarkKeys.has(key)) {
      generatedLandmarkKeys.add(key);
      appendPropBrushes([
        ...annularRingBrushes(origin, origin[2], origin[2] + 40, 59, 38, targetMaterials.concreteDark, 10),
        orientedBoxBrush(origin, yaw, [-50, -8, 38], [-42, 8, 126], targetMaterials.wood),
        orientedBoxBrush(origin, yaw, [42, -8, 38], [50, 8, 126], targetMaterials.wood),
        orientedBoxBrush(origin, yaw, [-54, -8, 116], [54, 8, 128], targetMaterials.wood),
        orientedBoxBrush(origin, yaw, [-58, -70, 128], [58, 70, 144], targetMaterials.corrugatedGray),
      ]);
      stats.landmarkAssemblies++;
    } else {
      stats.collapsedLandmarkParts++;
  ...
  if (substituted) {
    stats.sourcePropsSubstituted++;
    propSubstitutionRecords.push({
      model,
      origin: origin.map((coordinate) => Number(coordinate.toFixed(4))),
      angles: angles.map((coordinate) => Number(coordinate.toFixed(4))),
      scale,
      brushes: stats.propSubstituteBrushes - propBrushStart,
    });
  } else {
    stats.unsupportedPropsSkipped++;
    stats.unverifiedPropsOmitted++;
...
setcvar "g_obj_alliedtext2" "Direct VMF plus measured prop fill"
setcvar "g_obj_alliedtext3" "Windows, doors, roofs, cover, landmarks"
...
const report = {
  mapName,
  revision: 5,
  construction: "direct-source-brush-conversion-with-measured-prop-fill",
  displacementMode,
  compileBudgetMode,
  propPolicy: "measured-original-substitutes-omit-remainder",
  sourceBrushPolicy: "preserve playable world, func_detail, func_brush, and func_breakable solids; add only IDST-bounded original substitutes",
  ...
  modelBoundsManifest: "inferno-model-bounds.json",
  modelBoundsRecords: modelBoundsManifest.models.length,
  ...
  stats,
  propSubstitutions: propSubstitutionRecords,
};
fs.writeFileSync(
