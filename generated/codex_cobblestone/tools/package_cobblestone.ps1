param(
    ...
    [string]$MapName = "codex_cobblestone",
    [string]$GeneratedFolder = "generated_cobblestone",
    [switch]$SkipSource
)
...
try {
    ...
    try {
        $sourceEntries = @(
            ...
            @{ Source = (Join-Path $Workspace "work\mapgen\generate_cobblestone.js"); Entry = "$MapName/tools/generate_cobblestone.js" },
            @{ Source = (Join-Path $Workspace "work\mapgen\package_cobblestone.ps1"); Entry = "$MapName/tools/package_cobblestone.ps1" },
            @{ Source = (Join-Path $Workspace "work\mapgen\README-cobblestone.md"); Entry = "$MapName/README.md" }
        )
