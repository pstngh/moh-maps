...
$requiredEntries = @(
    ...
    @{ Source = (Join-Path $mainRoot "scripts\codex_cache.shader"); Entry = "scripts/codex_cache.shader" }
)

$textureRoot = Join-Path $mainRoot "textures\codex_cache"
$textureEntries = Get-ChildItem -LiteralPath $textureRoot -Filter "*.tga" -File |
    Sort-Object Name |
    ForEach-Object {
        @{
            Source = $_.FullName
            Entry = "textures/codex_cache/$($_.Name)"
        }
    }
